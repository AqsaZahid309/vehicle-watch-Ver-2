...
async def list_drivers(db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(get_current_user)):
...
async def create_driver(data: DriverCreate, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)):
...
async def update_driver(
    driver_id: uuid.UUID, data: DriverUpdate, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)
):
...
async def delete_driver(driver_id: uuid.UUID, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)):
...
async def get_trip(trip_id: uuid.UUID, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(get_current_user)):
    return await FleetService(db).get_trip(trip_id, user)


@router.get("/trips/{trip_id}/route", response_model=list[TripPoint])
async def trip_route(trip_id: uuid.UUID, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(get_current_user)):
...
async def list_geofences(db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(get_current_user)):
...
async def create_geofence(data: GeofenceCreate, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)):
...
async def update_geofence(
    geofence_id: uuid.UUID, data: GeofenceUpdate, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)
):
...
async def delete_geofence(geofence_id: uuid.UUID, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_manager)):
...
async def review_fuel_event(event_id: uuid.UUID, db: AsyncSession = Depends(get_db, scope="function"), user: User = Depends(require_operator)):
