...
@router.get("/active")
async def active_models(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)) -> list[dict[str, Any]]:
    return await MLService(db).active_models(user)


@router.post("/models/{version_id}/pin", response_model=ModelVersionRead)
async def pin_model(version_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(require_manager)):
...
async def unpin_model(version_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(require_manager)):
...
async def retrain(device_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(require_manager)):
...
async def detector_metrics(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
